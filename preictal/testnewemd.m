...
%mean
wholemean = mean(c)
%code for mean of imfs of signal after decomposition table IV
n=imf(:,1)';
my1 = mean(n)
n=imf(:,2)';
my2 = mean(n)
...
%code for standard deviation of whole signal table IV
wholestd=std(c)
%code for standard deviation of imfs table IV
ss=imf(:,1)';
sv1=std(ss)
...
%code for skewness of whole signal and resconstructed signal table IV
wholesk = skewness(c)
ss=imf(:,1)';
sk1=skewness(ss)
...
%Code for figure 6
  ...
  outfile = strcat('EMD_IMF_', int2str(ii), '.png')
  saveas(img, outfile);
end
...
xlabel('time(s)')
title('Reconstructed signal from EMD') ;
legend('Reconstructed Signal by EMD','Original EEG signal')
hold off;